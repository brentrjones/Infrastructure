...
echo -e "\e[32mLoading environment variables\n\e[31m\e[0m"
sleep 2
...
export biosfile=730-bios.ini
export raidfile=730-raid.ini
export idracfile=730-idrac.ini
export dtkxml=730-clone.xml
...
wget -q http://rpmrepo.domain/tftpboot/el6/dtk/templates/730-bios.ini -O $temppath/$biosfile
sleep 2
if [ $? != 0 ]; then
...
sleep 2
wget -q http://rpmrepo.domain/tftpboot/el6/dtk/templates/730-raid.ini -O $temppath/$raidfile
if [ $? != 0 ]; then
    echo -e "\e[31mWget Failed\e[0m"
    exit 1
fi
...
sleep 2
wget -q http://rpmrepo.domain/tftpboot/el6/dtk/templates/730-idrac.ini -O $temppath/$idracfile
if [ $? != 0 ]; then
    echo -e "\e[31mWget Failed\e[0m"
    exit 1
fi
...
wget -q http://rpmrepo.domain/tftpboot/el6/dtk/templates/730-clone.xml -O $temppath/$dtkxml
sleep 2
if [ $? != 0 ]; then
    echo -e "\e[31mWget Failed\e[0m"
    exit 1
...
echo -e \e[32mSetting random PSU\e[0m"
get_random=$((1 + RANDOM % 2))
sed -i.bak "s/SETME/$get_random/" $dtkxml
...
raidrep () {
    ...
    if [ ! -e $temppath/$raidfile ] ; then
        ...
        if [ $? != 0 ]; then
            echo -e "\e[31mRAID prep failed, exiting\e[0m"
            exit 1
        else
            sysrep
        fi
...
sysrep () {
    ...
    if [ ! -e $temppath/$dtkxml ] ; then
        ...
        if [ $? != 0 ]; then
            echo -e "\e[31mRACADM failed, exiting\e[0mm"
        else
            finish
        fi
    fi
